...
| 7 | MSVC fixture provisioning on Win10 VM: provision step added to `scripts/vm-provision.sh` (downloads `vs_buildtools.exe`, installs VCTools + Win10 SDK 19041 components via SYSTEM scheduled task, ~15-40 min cold). Fixture sources `testdata/testlib_msvc.{c,def}` + `build_testlib_msvc.cmd` produce a `/MD /GS /Gy` linked DLL with two `__declspec(dllexport)` symbols. Tests `TestPackBinary_FormatWindowsDLL_MSVC_E2E` and `_MSVC_Compress_E2E` pack the DLL, LoadLibrary it, GetProcAddress+invoke maldev_ping/maldev_add, assert outputs. | ~120 LOC + provision step | 🟢 in progress (MSVC installing on VM) |
