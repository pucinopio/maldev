...
func (m rootModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case SwitchToLicensesMsg:
		...
		filterMap := map[string]licenseFilter{
			...
			"superseded": licFilterSuperseded,
		}
	...
	case pushOverlayMsg:
		// Forward the current window size to the freshly pushed overlay so
		// renderers that depend on width/height (e.g. wizard progress bar)
		// don't get w=0 and panic.
		ov, _ := msg.overlay.Update(tea.WindowSizeMsg{Width: m.width, Height: m.hgt})
...
func (m rootModel) handleMouse(msg tea.MouseMsg) (tea.Model, tea.Cmd) {
	...
	// Only Press fires the action — Release is ignored. Listening to both
	// would double-fire and revert toggles (Press flips, Release flips back).
